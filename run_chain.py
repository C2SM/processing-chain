...
def run_chain(work_root, cfg, start_time, hstart, hstop, job_names, force):
    ...
    inidate_yyyy = start_time.strftime('%Y')
    inidate_mm = start_time.strftime('%m')
    inidate_dd = start_time.strftime('%d')
    inidate_hh = start_time.strftime('%H')
    ...
    setattr(cfg, 'inidate_yyyy', inidate_yyyy)
    setattr(cfg, 'inidate_mm', inidate_mm)
    setattr(cfg, 'inidate_dd', inidate_dd)
    setattr(cfg, 'inidate_hh', inidate_hh)
    ...
    if cfg.target is tools.Target.ICON or cfg.target is tools.Target.ICONART \
        ...
        setattr(
            ...
            os.path.join(cfg.icon_input_icbc,
                         os.path.basename(cfg.inidata_filename)))
        setattr(
