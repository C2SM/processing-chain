...
def era5_splitting_script(cfg, ERA5_folder, output_filenames):
    ...
    run_bash_script(era5_split_template, era5_split_job, cfg=cfg,
                    ml_files=ml_files, surf_files=surf_files, ERA5_folder=ERA5_folder)
...
def initial_conditions_script(cfg, ERA5_folder, CAMS_folder, era5_ini_file):
    ...
    run_bash_script(era5_ini_template, era5_ini_job, cfg=cfg,
                    era5_ml_file=era5_ml_file, era5_surf_file=era5_surf_file,
                    inicond_filename=era5_ini_file, ERA5_folder=ERA5_folder)
    ...
    run_bash_script(cams_ini_template, cams_ini_job, cfg=cfg,
                    inicond_filename=era5_ini_file, ERA5_folder=ERA5_folder,
...
def boundary_conditions_script(cfg, ERA5_folder, CAMS_folder, time):
    ...
    if not era5_nudge_file_final.exists():
        ...
        run_bash_script(nudging_template, nudging_job, cfg=cfg,
                        era5_ml_file=era5_ml_file, era5_surf_file=era5_surf_file,
                        filename=era5_nudge_file, ERA5_folder=ERA5_folder)
        ...
        run_bash_script(cams_nudge_template, cams_nudge_job, cfg=cfg,
                        filename=era5_nudge_file, ERA5_folder=ERA5_folder,
                        CAMS_file=CAMS_folder /
...
def create_icon_job(cfg, run_type, firstrun=False, runthrough=False):
    ...
    script_content = (fn := cfg.case_path / cfg.icon_runjob_filename).read_text().format(
        cfg=cfg,
        ...
        vprm_coeffs_nc=cfg.case_root / "global_inputs" / "VPRM" / cfg.CTDAS_global_inputs_VPRM[0].split('/')[-1],
...
def generate_tracers(cfg):
    ...
    TR_prior = generate_tracers_xml(cfg.tracers, cfg.CTDAS_nensembles, restart=False, propagate_bg=cfg.CTDAS_propagate_bg)
    TR_restart = generate_tracers_xml(cfg.tracers, cfg.CTDAS_nensembles, restart=True, propagate_bg=cfg.CTDAS_propagate_bg)
    ...
    if cfg.CTDAS_runthrough:
        TR_runthrough_prior = generate_tracers_xml(cfg.tracers, cfg.CTDAS_nensembles, cfg.CTDAS_nboundaries, restart=False, runthrough=True)
        TR_runthrough_restart = generate_tracers_xml(cfg.tracers, cfg.CTDAS_nensembles, cfg.CTDAS_nboundaries, restart=True, runthrough=True)
        with open(xml_folder / "tracers_runthrough_firstrun.xml", "w", encoding="utf-8") as file:
            file.write(TR_runthrough_prior)
        with open(xml_folder / "tracers_runthrough_restart.xml", "w", encoding="utf-8") as file:
...
def main(cfg):
    ...
    if cfg.chem_fetch_CAMS:
        tools.create_dir(CAMS_folder := cfg.case_root / "global_inputs" / "CAMS", "CAMS input files")
    ...
    if cfg.meteo_fetch_era5:
        tools.create_dir(ERA5_folder := cfg.case_root / "global_inputs" / "ERA5", "ERA5 input files")
        times = list(
        ...
        if not missing_files:
            ...
            logging.info(f"Missing files: {missing_files}. All data will be re-fetched.")
    ...
    if cfg.startdate_sim == cfg.startdate: create_icon_job(cfg, "opt2", firstrun=True)
    if cfg.CTDAS_runthrough: create_icon_job(cfg, "runthrough", runthrough=True)
    if (cfg.startdate_sim == cfg.startdate) and cfg.CTDAS_runthrough: create_icon_job(cfg, "runthrough", firstrun=True, runthrough=True)
    ...
    if cfg.startdate_sim == cfg.startdate:
        ...
        create_boundary_prior_all_ones(OEM_folder / 'boundary_lambdas_bg.nc',
                                       ...
                                       propagate_bg=cfg.CTDAS_propagate_bg)
        if cfg.CTDAS_runthrough:
                create_boundary_prior_separate(OEM_folder / 'boundary_lambdas_separate.nc',
                                    n_bg_ens=cfg.CTDAS_nboundaries)
    # Patch CTDAS files
    if cfg.startdate_sim == cfg.startdate:
        logging.info("Patching CTDAS files")
        def evaluate_dict(d, replace, using):
            return {key: eval(value.replace(replace, str(using))) for key, value in d.items()}
        meta_dict = {d.replace("XXX", "ENS"): {"ensemble": cfg.CTDAS_nensembles} if "XXX" in d else {} for d in cfg.tracers if not d.startswith("EM")}
        for key, source_paths in cfg.CTDAS["ctdas_patch"].items():
            ...
            for source_path in source_paths:
                ...
                with in_path.open('r') as infile, destination_path.open('w') as outfile:
