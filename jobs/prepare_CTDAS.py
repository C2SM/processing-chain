...
def main(cfg):
    ...
    if cfg.meteo_fetch_era5:
        ...
        era5_split_job = ERA5_folder / cfg.meteo_era5_splitjob
    ...
    era5_ini_file = ERA5_folder / f"era5_ini_{datestr}.nc"
    era5_ini_template = cfg.case_path / cfg.meteo_era5_inijob
    era5_ini_job = ERA5_folder / cfg.meteo_era5_inijob
    with open(era5_ini_template, 'r') as infile, open(era5_ini_job,
        ...
        outfile.write(infile.read().format(cfg=cfg,
                                           ...
                                           ERA5_folder=ERA5_folder))
    shutil.copy(cfg.case_path / 'mypartab', ERA5_folder / 'mypartab')
    ...
    cams_ini_job = ERA5_folder / cfg.chem_cams_inijob
    ...
    if cfg.meteo_interpolate_CAMS_to_ERA5:
        for time in tools.iter_hours(cfg.startdate_sim,
            ...
            cams_nudge_job = ERA5_folder / cfg.chem_cams_nudgingjob
    ...
    # The cells2stations and/or cells2cells thing can be computed on-the-fly with from sklearn.metrics.pairwise import haversine_distances. See test_cells2cells.py
