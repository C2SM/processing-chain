...
def create_oh_for_restart(cfg, month, ext_restart):
    ...
    for i, (k1, k2) in enumerate(zip(chunks[:-1], chunks[1:])):
        ...
        xlow, xhigh, alphalow, alphahigh = vertical_interp(
            pres_in.values, pres_out.values)
        ycoord = np.array(nlevels * [list(range(k2 - k1))])
        oh_chunk = oh.values[:, k1:k2]
        oh_regrid[:, k1:k2] = alphalow * oh_chunk[
            xlow, ycoord] + alphahigh * oh_chunk[xhigh, ycoord]

    oh_regrid = oh_regrid.unstack()
    oh_regrid = oh_regrid.transpose('time', 'layers_{}'.format(nlevels),
                                    'cells')

    M_moistair = M_DRYAIR * M_WATER / (qv_icon * M_DRYAIR +
                                       (1 - qv_icon) * M_WATER)
...
def create_oh_for_inicond(cfg, month):
    ...
    oh_regrid = oh_regrid * temp_era5.data * R_GAS / (N_AVO * 1e-6 *
                                                      pmid_era5.data)
