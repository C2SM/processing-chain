...
def main(date, indir, outdir, param):
    ...
    ifile = Dataset(
        os.path.join(indir, param["prefix"] + "_" + date.strftime('%Y-%m-%d') +
                     '.nc'), 'r')
    ...
    co2 = co2[:, :, lats_tf, :][:, :, :,
                                lons_tf] * 0.04401 / 0.02896  # * mmCO2/mmAir
    ...
    for ti, tt in enumerate(ttt):
        ...
        with Dataset(os.path.join(
                outdir,
                param["suffix"] + "_" + date.strftime('%Y%m%d') + tt + '.nc'),
                     mode='w') as ofile:
            ...
            oco2 = ofile.createVariable('CO2_BG',
                                        np.float32,
                                        ('date', 'level', 'lat', 'lon'),
                                        fill_value=-999.99)
            op = ofile.createVariable('pressure',
                                      np.float32,
                                      ('date', 'level', 'lat', 'lon'),
                                      fill_value=-999.99)
            osp = ofile.createVariable('PSURF',
                                       np.float32, ('date', 'lat', 'lon'),
                                       fill_value=-999.99)
            op0 = ofile.createVariable('P0',
                                       np.float32, ('date'),
                                       fill_value=-999.99)
            ...
            otime.units = 'seconds since ' + date.strftime(
                '%Y%m%d') + ' 00:00:00'
