...
def main(cfg):
    ...
    shutil.copy(
        os.path.join(cfg.case_path, 'partab_LAM'),
    ...
    if cfg.lrestart == '.FALSE.':
        ...
        date_icon_out_format = cfg.startdate_sim.strftime(cfg.LAM_icon_output_nameformat)
        
        # -- Copy IC remapping script in workdir
        with open(
            os.path.join(cfg.case_path, 'icon_LAM_ic.sh')
        ) as inf:
            to_write = inf.read()
    ...
    for time in tools.iter_hours(cfg.startdate_sim, cfg.enddate_sim,
                                 cfg.LAM_inc):
        icon_output_file = os.path.join(cfg.LAM_icon_output_dir, 
                                  cfg.LAM_prefix + 
                                  "LBC_" + 
    ...
    with open(
        os.path.join(cfg.case_path, 'icon_LAM_lbc.sh')
    ) as inf:
        to_write = inf.read()
    ...
    process = subprocess.Popen([
        "bash",
        os.path.join(cfg.icon_input_icbc, 'icon_LAM_lbc.sh')
    ],
                            stdout=subprocess.PIPE)
    process.communicate()
    ...
    for time in tools.iter_hours(cfg.startdate_sim, cfg.enddate_sim,
                                 cfg.LAM_inc):
        old_name = os.path.join(cfg.icon_input_icbc, 
                                cfg.LAM_prefix + 
                                "LBC_" +
                                time.strftime(cfg.LAM_icon_output_nameformat) +
                                '_tmp' + 
        ...
        new_name = os.path.join(cfg.icon_input_icbc, 
                                cfg.lbcdata_prefix + 
                                time.strftime(cfg.lbcdata_nameformat) +
                                '_tmp' + 
                                cfg.lbcdata_filename_suffix
                                )
    ...
    var_list = ['clon','clon_bnds','clat','clat_bnds','height','height_bnds','time']
    ...
    if cfg.lrestart == '.FALSE.':
        ...
        out_chem = os.path.join(
            cfg.art_input_folder,
            cfg.chem_ini_filename
        )
        out_aero = os.path.join(
            cfg.art_input_folder,
            cfg.aero_ini_filename
        )
        ...
        logging.info(
                    "Created IC files {} and {}".format(out_chem, out_aero))
    ...
    lbc_list = ['ALKNO3_full',
                ...
                'clon','clon_bnds',
                'depth',
                'depth_2','depth_2_bnds',
                'height','height_bnds',
                'height_2',
                'height_3','height_3_bnds',
                'height_4',
                'height_5',
                'height_6','height_6_bnds',
                'time'] 

    for time in tools.iter_hours(cfg.startdate_sim, cfg.enddate_sim,
                                 cfg.LAM_inc):
        in_file = os.path.join(cfg.icon_input_icbc, 
                                cfg.lbcdata_prefix + 
                                time.strftime(cfg.lbcdata_nameformat) +
                                '_tmp' + 
                                cfg.lbcdata_filename_suffix
                                )
        out_file = os.path.join(cfg.icon_input_icbc, 
                                cfg.lbcdata_prefix + 
                                time.strftime(cfg.lbcdata_nameformat) +
                                cfg.lbcdata_filename_suffix
                                )
