...
def main(starttime,hstart,hstop,cfg):
    ...
    dest_folder = os.path.join(cfg.cosmo_input,"obs_nudging")
    try:
    ...
    count = 0  # running index for COSMO
    for time in tools.iter_hours(starttime, hstart, hstop, step=24):
        ...
        for prefix in cfg.obs_nudging_prefixes:
            ...
            src_file = os.path.join(cfg.obs_nudging_dir, src_filename)
            if count == 1:
                dest_path = os.path.join(dest_folder, prefix)
            else:
                dest_path = os.path.join(dest_folder, "{}.{}".format(prefix, count))
