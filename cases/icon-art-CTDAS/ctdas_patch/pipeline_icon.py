...
def ensemble_smoother_pipeline(dacycle, platform, dasystem, samples,
                               statevector, obsoperator, optimizer):
...
def forward_pipeline(dacycle, platform, dasystem, samples, statevector,
                     obsoperator):
    """ The main point of entry for the pipeline """
    sys.path.append(os.getcwd())
    ...
    if 'forward.savestate.exceptsam' in dacycle:
        sam = (dacycle['forward.savestate.exceptsam'].upper()
               in ["TRUE", "T", "YES", "Y"])
    else:
        sam = False
    ...
    if 'forward.savestate.legacy' in dacycle:
        legacy = (dacycle['forward.savestate.legacy'].upper()
                  in ["TRUE", "T", "YES", "Y"])
    else:
        legacy = False
    ...
    if not fwddir:
        ...
        if sam:
            filename = os.path.join(
                fwddir, dacycle['time.start'].strftime('%Y%m%d'),
                'savestate_%s.nc' % dacycle['time.start'].strftime('%Y%m%d'))
            ...
            filename = os.path.join(fwddir,
                                    dacycle['time.start'].strftime('%Y%m%d'),
                                    'savestate.nc')
    ...
    if not fwddir:
        ...
        logging.info("Running forward with prior savestate from: %s" %
                     savefilename)
        ...
        logging.info("Running forward with optimized savestate from: %s" %
                     filename)
...
def prepare_state(dacycle, statevector):
    ...
    if 'inversion.savestate.dir' in dacycle:
        ...
        method = dacycle[
            'inversion.savestate.method']  # valid options: read_new_member and read_mean
        logging.info(
            'Ensemble members will be initialized from optimized ensembles in %s'
            % initdir)
    else:
    ...
    if not dacycle['time.restart']:
        ...
        for n in range(statevector.nlag):
            ...
            if method == 'create_new_member':
                date = dacycle['time.start'] + datetime.timedelta(
                    days=(n + 0.5) * int(dacycle['time.cycle']))
                cov = statevector.get_covariance(date, dacycle)
                statevector.make_new_ensemble(
                    n, cov, int(dacycle['statevector.bg_params']))

            elif method == 'read_new_member' or method == 'read_mean':
                date = dacycle['time.start'] + datetime.timedelta(
                    days=n * int(dacycle['time.cycle']))
                filename_new_member = os.path.join(
                    initdir, date.strftime('%Y%m%d'),
                    'savestate_%s.nc' % date.strftime('%Y%m%d'))
                ...
                if os.path.exists(filename_new_member):
                    if method == 'read_new_member':
                        statevector.read_ensemble_member_from_file(
                            filename_new_member, n, qual='opt', read_lag=0)
                    elif method == 'read_mean':
                        date = dacycle['time.start'] + datetime.timedelta(
                            days=(n + 0.5) * int(dacycle['time.cycle']))
                        cov = statevector.get_covariance(date, dacycle)
                        meanstate = statevector.read_mean_from_file(
                            filename_new_member, n, qual='opt')
                    ...
                    if method == 'read_new_member':
                        ...
                        meanstate = statevector.read_mean_from_file(
                            filename_new_member, n, date, initdir, qual='opt')
                        date = dacycle['time.start'] + datetime.timedelta(
                            days=(n + 0.5) * int(dacycle['time.cycle']))
                        cov = statevector.get_covariance(date, dacycle)
                        statevector.make_new_ensemble(n, cov, meanstate)
        ...
        saved_sv = os.path.join(
            dacycle['dir.restart'], 'savestate_%s.nc' %
            dacycle['da.restart.tstamp'].strftime('%Y%m%d'))
        ...
        if method == 'create_new_member':
            ...
            date = dacycle['time.start'] + datetime.timedelta(
                days=(statevector.nlag - 1) * int(dacycle['time.cycle']))
            filename_new_member = os.path.join(
                initdir, date.strftime('%Y%m%d'),
                'savestate_%s.nc' % date.strftime('%Y%m%d'))
            statevector.propagate(dacycle, method, filename_new_member, date,
                                  initdir)

    # Finally, also write the statevector to a file so that we can always access the a-priori information
    current_sv = os.path.join(
        dacycle['dir.restart'],
        'savestate_%s.nc' % dacycle['time.start'].strftime('%Y%m%d'))
...
def sample_state(dacycle, samples, statevector, obsoperator):
    ...
    for lag in range(nlag):
        logging.info(header + ".....Ensemble Kalman Filter at lag %d" %
                     (lag + 1))
...
def sample_step(dacycle,
    ...
    dacycle['time.sample.stamp'] = "%s_%s" % (startdate.strftime("%Y%m%d%H"),
                                              enddate.strftime("%Y%m%d%H"))

    logging.info("New simulation interval set : ")
    logging.info("                  start date : %s " %
                 startdate.strftime('%F %H:%M'))
    logging.info("                  end   date : %s " %
                 enddate.strftime('%F %H:%M'))
    logging.info("                  file  stamp: %s " %
                 dacycle['time.sample.stamp'])
    ...
    for sample in samples:
        ...
        sampling_coords_file = os.path.join(
            ...
            '_coordinates_%s.nc' % dacycle['time.sample.stamp'])
        sample.write_sample_coords(sampling_coords_file)

        # Write filename to dacycle, and to output collection list
        dacycle['ObsOperator.inputfile.' +
                sample.get_samples_type()] = sampling_coords_file
    ...
    for i in range(len(samples)):
        if os.path.exists(dacycle['ObsOperator.inputfile.' +
                                  samples[i].get_samples_type()]):
    ...
    if not advance:
        if dacycle['time.restart'] == False or lag == int(
                dacycle['time.nlag']) - 1:
...
def invert(dacycle, statevector, optimizer):
    ...
    if statevector.nobs <= 1:  #== 0:
        logging.info(
            'List with observations to assimilate is empty, skipping invert step and continuing without statevector update...'
        )
        return

    dims = (int(dacycle['time.nlag']), int(dacycle['da.optimizer.nmembers']),
            int(dacycle.dasystem['nparameters']), statevector.nobs)
    ...
    diagnostics_file = os.path.join(
        dacycle['dir.output'],
        'optimizer.%s.nc' % dacycle['time.start'].strftime('%Y%m%d'))
...
def advance(dacycle, samples, statevector, obsoperator):
    ...
    for sample in samples:
        dacycle.output_filelist.append(dacycle['ObsOperator.inputfile.' +
                                               sample.get_samples_type()])
        logging.debug(
            "Appended Observation filename to dacycle for collection: %s" %
        ...
        if os.path.exists(sampling_coords_file):
            ...
            logging.warning(
                "Sample auxiliary output not written, because input file does not exist (no samples found in obspack)"
            )
    del sample
