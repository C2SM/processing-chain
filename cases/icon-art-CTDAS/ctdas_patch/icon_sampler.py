...
parser.add_argument("--iconout_prefix", type=str,
                    help="Headings of the ICON output files")
parser.add_argument("--icon_grid", type=str,
...
logfile = os.path.join(wd, "log/iconout_sampler." + str(settings['nproc']) + ".log")
...
iconhelper.validate_settings(['sampling_coords_file',
                              'run_dir',
                              'iconout_prefix',
                              'icon_grid',
                              'nproc',
...
dat = iconhelper.read_sampling_coords(settings['sampling_coords_file'], id0, id1)
...
# Query BallTree
(d,i) = tree.query(np.column_stack([np.deg2rad(lat_q), np.deg2rad(lon_q)]), k=nneighb, return_distance=True)
...
for idx in range( len(dat['latitude']) ):
    ...
    if len(tmp) == 1:
        ...
        frac_t[idx] = (time1 - dat['time'][idx]).total_seconds() / (time1 - time0).total_seconds()
        ...
        if dat['time'][idx] == iconout_times[-1]:
            ...
            msg = "Sample %d, sounding_id %s: outside of simulated time."%(idx, dat['sounding_id'][idx])
...
# -- Begin Sampling
ens_sim, prior = iconhelper.sample_total_columns_ICON(dat, loc_input, member_names)
...
os.system("echo 'Writing output file '" + os.path.join(iconhelper.settings['run_dir'], outfile) + " >> " + logfile)
