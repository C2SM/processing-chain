...
class ObservationOperator(object):
    ...
    def make_lambdas(self,statevector,lag):
        ...
        if statevector.isOptimized:
            self.lambda_file = os.path.join(self.simulationdir,'global_inputs','OEM','lambda_%s_opt2.nc' %self.dacycle['time.sample.stamp'][0:8])
            self.bg_lambda_file = os.path.join(self.simulationdir,'global_inputs','OEM','bg_lambda_%s_opt2.nc' %self.dacycle['time.sample.stamp'][0:8])
        else:
            if lag==0:
                self.lambda_file = os.path.join(self.simulationdir,'global_inputs','OEM','lambda_%s_opt1.nc' %self.dacycle['time.sample.stamp'][0:8])
                self.bg_lambda_file = os.path.join(self.simulationdir,'global_inputs','OEM','bg_lambda_%s_opt1.nc' %self.dacycle['time.sample.stamp'][0:8])
            else:
                self.lambda_file = os.path.join(self.simulationdir,'global_inputs','OEM','lambda_%s_prior.nc' %self.dacycle['time.sample.stamp'][0:8])
                self.bg_lambda_file = os.path.join(self.simulationdir,'global_inputs','OEM','bg_lambda_%s_prior.nc' %self.dacycle['time.sample.stamp'][0:8])
        ...
        if {cfg.CTDAS_propagate_bg}:
            for ireg in range(0,nr_reg):
                for icat in range(0,nr_cat):
                    lambdas[-1,ireg,icat,0] = 0.0 # Set anthropogenic component to 0
        odata[:] = lambdas
        ofile.close()
        ...
        if {cfg.CTDAS_propagate_bg}:
            for idir in range(0,nr_dir):
                lambdas[-1,idir] = lambdas[-2,idir] # Populate BG lambdas with the last member (which, for an optimized run, is the optimized member)
        odata[:] = lambdas
        ofile.close()
    ...
    def run(self,samples,statevector,lag):
        ...
        time = dt.datetime.strptime(self.dacycle['time.sample.stamp'][0:10], "%Y%m%d%H")
        ...
        if statevector.isOptimized:
            runscript = os.path.join(self.simulationdir,folder_timestr,'icon','run','{ (cfg.case_path / cfg.icon_runjob_filename).stem }' + '_%s_opt2.job'%(self.dacycle['time.sample.stamp'][0:8]))
            self.outfolder = os.path.join('{cfg.case_root / "global_outputs"}', f"opt2_{{job_timestr}}")
            finalfile = os.path.join(self.outfolder, f"ICON-ART-OEM-INIT_{{(time + dt.timedelta(seconds={cfg.CTDAS_restart_init_time}) + dt.timedelta(days={cfg.CTDAS_ctdas_cycle})).strftime('%Y-%m-%dT%H:%M:%S')}}.000.nc")
        else:
            if lag==0:
                runscript = os.path.join(self.simulationdir,folder_timestr,'icon','run','{ (cfg.case_path / cfg.icon_runjob_filename).stem }' + '_%s_opt1.job'%(self.dacycle['time.sample.stamp'][0:8]))
                self.outfolder = os.path.join('{cfg.case_root / "global_outputs"}', f"opt1_{{job_timestr}}")
                finalfile = os.path.join(self.outfolder, f"ICON-ART-OEM-INIT_{{(time + dt.timedelta(seconds={cfg.CTDAS_restart_init_time}) + dt.timedelta(days={cfg.CTDAS_ctdas_cycle})).strftime('%Y-%m-%dT%H:%M:%S')}}.000.nc")
            else:
                runscript = os.path.join(self.simulationdir,folder_timestr,'icon','run','{ (cfg.case_path / cfg.icon_runjob_filename).stem }' + '_%s_prior.job'%(self.dacycle['time.sample.stamp'][0:8]))
                self.outfolder = os.path.join('{cfg.case_root / "global_outputs"}', f"prior_{{job_timestr}}")
                finalfile = os.path.join(self.outfolder, f"ICON-ART-OEM-INIT_{{(time + dt.timedelta(seconds={cfg.CTDAS_restart_init_time}) + dt.timedelta(days={cfg.CTDAS_ctdas_cycle})).strftime('%Y-%m-%dT%H:%M:%S')}}.000.nc")
...
###########################################################
        ...
        meta["temp"] = {{}}
        meta["qv"] = {{}}
        outfile = os.path.join(self.simulationdir,"global_outputs","extracted_ICOS",'%s%s.nc'%(prefix,job_timestr))
        ...
        logging.info(f"Running ICON sampler with starttime {{starttime}} and endtime {{endtime}} and obsdir {{obs_dir}} and nneighb {{nneighb}} and meta {{meta}} and outfile {{outfile}}")
        ICON_sampler(infolder, "ICON-ART-UNSTR", "{cfg.input_files_scratch_dynamics_grid_filename}", starttime, endtime, obs_dir, nneighb, meta, outfile, mountain_stations=mountain_stations)
        ...
        TR_A_ENS = (molar_mass['da']/molar_mass[self.tracer])*units_factor[self.tracer]*np.array(f1.get_variable('TR'+self.tracer.upper()+'_A_ENS') + f1.get_variable('biosource') - f1.get_variable('biosink')) #float CH4_A_ENS(ens, sites, time) 1 --> ppb
        ...
        for iobs in np.arange(len(obs)):
            ...
            if station_name not in mdm_dictionary.keys(): continue # Skip stations that aren't considered
