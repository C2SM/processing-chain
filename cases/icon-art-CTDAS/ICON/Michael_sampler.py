...
def intp_icon_data(iloc, gridinfo, datainfo, latitudes, longitudes, asl, elev,
                   station_name, mountain_stations):
...
def ICON_sampler(DATA_path,
    ...
    class gridinfo:
        ...
        neighbor_cell_index = np.array(
            fh_grid.variables['neighbor_cell_index'])
    ...
    n_ens = int(
        np.nansum([1 for var in meta.keys() if 'ensemble' in meta[var]]))
    ...
    def process_data(index, gridinfo, datainfo, latitudes, longitudes, asl,
                     elev, obsnames, mountain_stations, results):
        result = intp_icon_data(index, gridinfo, datainfo, latitudes,
                                longitudes, asl, elev, obsnames,
                                mountain_stations)
    ...
    while looptime <= enddate:
        ...
        DATA_file = os.path.join(
            DATA_path, '%s_%s:00:00.000.nc' % (fname_base, timestring))
        ...
        for var in meta.keys():
            if 'ensemble' in meta[var]:
                ...
                for iens in np.arange(meta[var]['ensemble']):
                    ...
                    ICON_data_ens[ivar, iens,
                                  ...] = np.array(fh_data.variables[varnc])
                ivar += 1
    #%%
            ...
            for i in range(len(latitudes)):
                p = Process(target=process_data,
                            args=(i, gridinfo, datainfo, latitudes, longitudes,
                                  asl, elev, obsnames, mountain_stations,
                                  results))
                processes.append(p)
                p.start()
    ...
    with Dataset(outfile, mode='w') as ofile:
        ...
        for var in meta.keys():
            if 'ensemble' in meta[var]:
                ...
                ovar = ofile.createVariable(varnc, np.float32,
                                            ('ens_%.2i' %
                                             (ivar + 1), 'sites', 'time'))
                ovar[:, :, :] = intp_ICON_data_ens[
                    ivar, 0:meta[var]['ensemble'], :, :]
                ivar += 1
...
if __name__ == '__main__':
    sample_ICON = ICON_sampler(
        "/capstor/scratch/cscs/ekoene/processing-chain/work/icon-art-CTDAS/global_outputs/prior_20180101",
        "ICON-ART-UNSTR",
        "/capstor/scratch/cscs/ekoene/processing-chain/work/icon-art-CTDAS/global_inputs/grid/icon_europe_DOM01.nc",
        "2018-01-01 00:00:00",
        "2018-01-12 00:00:00",
        "/capstor/scratch/cscs/ekoene/processing-chain/work/icon-art-CTDAS/global_inputs/ICOS",
        5, {
            'TRCO2_A': {},
            'TRCO2_BG': {},
            'CO2_RA': {},
        ...
        nlev=60,
        mountain_stations=[
            'Jungfraujoch_13', 'Monte Cimone_8', 'Puy de Dome_10',
