...
# starttime = {starttime}
# enddtime = {endtime}
# obs_dir = {observation_dir}
# nlev = 60 #number of vertical levels
# nneighb = 5 #number of nearest neighbors to consider
# n_member = {n_member}
# meta = { 
#         'TRCO2_A': {'offset': 0.},
#         'TRCO2_BG': {'offset': 0.},
...
#         'qv': {'offset': 0.},
#         'temp': {'offset': 0.},
#         # 'biosink_chemtr': {'offset': 0.},
#         # 'biosource_all_chemtr': {'offset': 0.},
#         'TRCO2_A-ENS': {'offset': 0, 'ensemble': n_member},
#         }
# outfile = {outfile}
...
def intp_icon_data(iloc, gridinfo, datainfo, latitudes, longitudes, asl, elev, station_name, mountain_stations):
    ...
    for nnidx in np.arange(len(nn_sel)):
        if idx_below[nnidx] != idx_above[nnidx]:
            vert_scaling_fact[nnidx] = (target_asl[nnidx]-datainfo.z_mc[idx_below[nnidx],nn_sel[nnidx]])/(datainfo.z_mc[idx_above[nnidx],nn_sel[nnidx]]-datainfo.z_mc[idx_below[nnidx],nn_sel[nnidx]])
        else:
...
def ICON_sampler(DATA_path, fname_base, ICON_grid, starttime, endtime, obs_dir, nneighb, meta, outfile, nlev=60, mountain_stations=[]):
    ...
    for ncfile in os.listdir(obs_dir):
        ...
        if not startdate.strftime('%Y%m%d') in ncfile: continue
        ...
        for x in st_ind:
            ...
            elev.append( float(obsnames[-1].split('_')[-1]) )
    ...
    def process_data(index, gridinfo, datainfo, latitudes, longitudes, asl, elev, obsnames, mountain_stations, results):
        result = intp_icon_data(index, gridinfo, datainfo, latitudes, longitudes, asl, elev, obsnames, mountain_stations)
    ...
    while looptime <= enddate:
        ...
        for var in meta.keys():
            if not 'ensemble' in meta[var]:
                ICON_data_det[ivar,...] = np.array(fh_data.variables[var])
                ivar+=1
        ivar = 0
        for var in meta.keys():
            if 'ensemble' in meta[var]:
    #            for iens in np.arange(n_member):
                for iens in np.arange(meta[var]['ensemble']):
                    ...
                    ICON_data_ens[ivar,iens,...] = np.array(fh_data.variables[varnc])
                ivar+=1
    #%%
        ...
        if looptime == startdate:
            ...
            for i in range(len(latitudes)):
                p = Process(target=process_data, args=(i, gridinfo, datainfo, latitudes, longitudes, asl, elev, obsnames, mountain_stations, results))
                processes.append(p)
                p.start()
    ...
    #%%
    """Save as netcdf"""
    with Dataset(outfile, mode='w') as ofile:
...
if __name__ == '__main__':
    sample_ICON = ICON_sampler("/capstor/scratch/cscs/ekoene/processing-chain/work/icon-art-CTDAS/global_outputs/prior_20180101", 
                           "ICON-ART-UNSTR", 
                           "/capstor/scratch/cscs/ekoene/processing-chain/work/icon-art-CTDAS/global_inputs/grid/icon_europe_DOM01.nc", 
                           "2018-01-01 00:00:00", "2018-01-12 00:00:00", "/capstor/scratch/cscs/ekoene/processing-chain/work/icon-art-CTDAS/global_inputs/ICOS", 5, 
                           {'TRCO2_A': {}, 'TRCO2_BG': {}, 'CO2_RA': {}, 'CO2_GPP': {}, 'TRCO2_A-ENS': {'ensemble': 186}, 'biosource': {}, 'biosink': {}, 'u': {}, 'v': {}, 'temp': {}, 'qv': {}}, 
                           "/capstor/scratch/cscs/ekoene/processing-chain/work/icon-art-CTDAS/global_outputs/extracted_ICOS/prior_20180101.nc",
                           nlev=60, mountain_stations=['Jungfraujoch_13', 'Monte Cimone_8', 'Puy de Dome_10', 'Pic du Midi_28', 'Zugspitze_3', 'Hohenpeissenberg_50', 'Hohenpeissenberg_93', 'Hohenpeissenberg_131', 'Schauinsland_12', 'Plateau Rosa_10'])
