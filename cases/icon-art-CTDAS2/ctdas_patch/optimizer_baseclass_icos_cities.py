...
class Optimizer(object):
    ...
    def create_matrices(self):
        ...
        self.coeff_matrix = np.exp(-ds.Distances.values/400).T      # ERIK: I set this to 400 as a rough footprint size for a station (was 600 km for Michael; 60 km for Nikolai)
    ...
    def state_to_matrix(self, statevector):
        ...
        allfromfiles = [] # collect all evns for finding loc_coeffs in localize()
        ...
        for n in range(self.nlag):
            ...
            if samples != None:
                ...
                for m in range(len(samples)):
                    ...
                    logging.info('Lag %i, sample %i: rejection_threshold = %i, nobs = %i' %(n, m, sample.rejection_threshold, sample.getlength()))
                    ...
                    try:
                        ...
                        logging.debug(f"Number of copies: {{len(sample.getvalues('lat'))}}")
                        allevns.extend(['column']*len(sample.getvalues('lat')))
                        allfromfiles.extend(['column']*len(sample.getvalues('lat')))
        ...
        OBSERVATIONS_IN_RADIANS_LATLON = np.deg2rad(np.column_stack([self.latitude,self.longitude]))
        grid = xr.open_dataset('/users/ekoene/CTDAS_inputs/icon_europe_DOM01.nc')
        grid_latitudes = grid.lat_cell_centre.values
        grid_longitudes = grid.lon_cell_centre.values
        REGIONS_IN_RADIANS_LATLON = np.column_stack([grid_latitudes,grid_longitudes])
        Distances = haversine_distances(OBSERVATIONS_IN_RADIANS_LATLON,REGIONS_IN_RADIANS_LATLON) * 6371000/1000 # distance to km s
        logging.debug(Distances)
        self.coeff_matrix = np.exp(-Distances/400)      # ERIK: I set this to 400 as a rough footprint size for a station (was 600 km for Michael; 60 km for Nikolai)
        self.name_array = np.arange(OBSERVATIONS_IN_RADIANS_LATLON.shape[0]) # These should be 'names' but my pixels don't have names, of course!
    ...
    def serial_minimum_least_squares(self,n_bg_params=0):
        ...
        for n in range(self.nobs):
            ...
            if self.flags[n] != 0:
                ...
                logging.info('Skipping observation (%s,%i) because of flag value %d' % (self.sitecode[n], self.obs_ids[n], self.flags[n]))
                continue
            ...
            logging.info('Proceeding to assimilate observation %s, %i' % (self.sitecode[n], self.obs_ids[n]))
            ...
            if self.may_localize[n]:
                ...
                logging.info('Trying to localize observation %s, %i' % (self.sitecode[n], self.obs_ids[n]))
...
###                logging.debug('Localized observation %i with localization length %s' %(self.obs_ids[n], self.loc_L[n]))
            ...
            n_em_cat = 2
            if self.fromfile[n] in skip_stations: return # Skip stations outside of the domain!
            ...
            logging.info('Localized observation %i at station %s (nr. %i)'%(self.obs_ids[n],self.fromfile[n], n))
