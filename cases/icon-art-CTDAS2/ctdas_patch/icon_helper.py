...
# Here are some adaptations written by David Ho
    ...
    def get_divisible_hours_string(datetime_obj, hours=3):
        ...
        if hour % hours == 0:
            ...
            return [current_hour.strftime('%Y%m%d%H'), hour_above.strftime('%Y%m%d%H')]
        else:
            ...
            return [hour_below.strftime('%Y%m%d%H'), hour_above.strftime('%Y%m%d%H')]
    ...
    def sample_total_columns_ICON(self, dat, loc, fields_list):
        ...
        for time_id in UT:
            ...
            if do_CAMS:
                CAMS = xr.open_mfdataset(["/scratch/snx3000/ekoene/CAMS_i/cams_egg4_"+hour_below+".nc",
                                          "/scratch/snx3000/ekoene/CAMS_i/cams_egg4_"+hour_above+".nc"],
                                          concat_dim="Time",
                                          combine="nested").rename({{'Time': 'time'}})
            ...
            biosource_all_chemtr = np.asarray(self._read_and_intrp_v_ICON(loc, ['biosource_all_chemtr'], time_id, idt)) / (1-qv) * (m_dry/m_gas) * to_ppm
            biosink_chemtr = np.asarray(self._read_and_intrp_v_ICON(loc, ['biosink_chemtr'], time_id, idt)) / (1-qv) * (m_dry/m_gas) * to_ppm
