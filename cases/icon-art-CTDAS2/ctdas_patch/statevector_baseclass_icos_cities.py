...
class StateVector(object):
    ...
    def setup(self, dacycle):
        ...
        self.nmembers = int(
            dacycle['da.optimizer.nmembers']
        )  #number of ensemble members, e.g. 192 for the icon case
        self.nparams = int(dacycle.dasystem['nparameters']
                           )  #n_reg * n_tracers * n_categories + n_bg_params
        self.nobs = 0
        ...
        self.distances = dacycle['sv.distances']
        #---------
    ...
    def make_new_ensemble(self, lag, covariancematrix=None, n_bg_params=0):
        ...
        if np.all(self.C == 0.):
            ...
            covariancematrix = np.zeros((self.nparams, self.nparams),
                                        dtype=np.float32)
            # covariancematrix = np.zeros((self.nparams,self.nparams))
            # print("COV=", covariancematrix.shape)
            specific_length_bio = 300
            specific_length_anth = 200
            # print("dist=", self.distances)

            ds = xr.open_dataset(self.distances)
            logging.info('opened distances file, nparams = %d' % self.nparams)
            distances = ds.Distances.values
            # covariancematrix[:-n_bg_params, :-n_bg_params] = np.kron(np.exp(-distances/specific_length), c), c is the correlation matrix between categories
            # for ix, x in enumerate(distances):
            #     if ix<1:
            #         covariancematrix[0,0:-n_bg_params][::3] = 1.*np.exp(-x/specific_length)
            ...
            #     else:
            #         covariancematrix[3*(ix)+0,0:-n_bg_params][::3] = 1.*np.exp(-x/specific_length)
            ...
            for ix, x in enumerate(distances):
                for ic, c in enumerate(Corr):
                    for ik, k in enumerate(c):
                        if ic == 1 or ik == 1:
                            covariancematrix[
                                ix * categories + ic,
                                ik:-n_bg_params][::categories] = 0.5 * np.exp(
                                    -x / specific_length_anth) * k
                        else:
                            covariancematrix[
                                ix * categories + ic,
                                ik:-n_bg_params][::categories] = 1 * np.exp(
                                    -x / specific_length_bio) * k
            ...
            #set variances for the 8 background elements (note python indexing) (10% std in this case):
            if n_bg_params > 0:
                for iii in np.arange(n_bg_params):
                    covariancematrix[
                        -n_bg_params + iii, -n_bg_params +
                        iii] = 0.015 * 0.015  # 0.015*400 = 6 ppm stdev
                    covariancematrix[-n_bg_params +
                                     np.mod(iii + 1, n_bg_params),
                                     -n_bg_params + iii] = 0.015 * 0.015 * 0.25
                    covariancematrix[-n_bg_params +
                                     np.mod(iii - 1, n_bg_params),
                                     -n_bg_params + iii] = 0.015 * 0.015 * 0.25
            #logging.info('Filled in cov matrix, dtype %s, %s, %d, %d'%(str(covariancematrix.dtype),str(covariancematrix[0][0].dtype), covariancematrix.shape[0], covariancematrix.shape[1]) )
            self.C = np.linalg.cholesky(covariancematrix)
            del covariancematrix

#           # covariancematrix[covariancematrix<1.2e-2] = 0.
#
#        #set variances for lbc-scaling
##        for idir in np.arange(4):
##          covariancematrix[-(idir+1),-(idir+1)] = 0.5

# msteiner: commented-out the svd as it takes endless time for a large statevector and
#... it is just for information about the dof
...
#        dof = np.sum(s) ** 2 / sum(s ** 2)
        ...
        newmean = np.ones(self.nparams,
                          float)  # standard value for a new time step is 1.0
        if lag == self.nlag - 1 and self.nlag >= 2:
            ...
            if n_bg_params > 0:
                newmean[self.nparams - n_bg_params:] = self.ensemble_members[
                    lag - 1][0].param_values[self.nparams - n_bg_params:]
        ...
        for cat in range(categories):
            ...
            logging.info('New mean (%s) ' %
                         str(np.nanmean(newmean[cat:][::categories])))
        ...
        for member in range(1, self.nmembers):
            ...
            newmember.param_values = np.einsum("ij, j -> i", self.C,
                                               rands) + newmean
            logging.info('post-dot')
            self.ensemble_members[lag].append(newmember)
            logging.info('Created parameters for ensemble member %i' %
                         (member))
        ...
        for member in range(0, self.nmembers):
            logging.info(
                'Member shape (%s) ' %
                str(np.shape(self.ensemble_members[lag][member].param_values)))
            lambdas = np.append(
                lambdas, self.ensemble_members[lag][member].param_values)
        ...
        for cat in range(categories):
            logging.info('Category (%s) ' % str(cat + 1))
            logging.info('Lambda mean (%s) ' %
                         str(np.nanmean(members_array[cat:][::categories])))
    ...
    def propagate(self,
        ...
        if method == 'create_new_member':
            date = dacycle['time.start'] + timedelta(
                days=(self.nlag - 0.5) * int(dacycle['time.cycle']))
            cov = self.get_covariance(date, dacycle)
            self.make_new_ensemble(self.nlag - 1, cov,
                                   int(dacycle['statevector.bg_params']))

        elif method == 'read_new_member':
            if os.path.exists(filename):
                self.read_ensemble_member_from_file(filename,
                                                    self.nlag - 1,
                                                    qual='opt',
                                                    read_lag=0)
            else:
            ...
            cov = self.get_covariance(date, dacycle)
            if os.path.exists(filename):
                meanstate = self.read_mean_from_file(filename,
                                                     self.nlag - 1,
                                                     qual='opt')
            else:
                meanstate = self.read_mean_from_file(filename,
    ...
    def write_to_file(self, filename, qual):
        ...
        if qual == 'prior':
            ...
            logging.debug('Creating new StateVector output file (%s)' %
                          filename)
            #qual = 'prior'
        else:
            f = io.CT_CDF(filename, method='write')
            logging.debug('Opening existing StateVector output file (%s)' %
                          filename)
            #qual = 'opt'
        ...
        for n in range(self.nlag):
            ...
            savedict['count'] = n
            savedict[
                'comment'] = 'this represents deviations from the mean of the ensemble'
            f.add_data(savedict)
        f.close()

        logging.info('Successfully wrote the State Vector to file (%s) ' %
                     filename)

    def interpolate_mean_ensemble(self,
                                  initdir,
                                  date,
                                  qual='opt',
                                  readensemble=True):
        ...
        for d in range(ddays):
            ...
            if not found_datemin and os.path.exists(
                    os.path.join(
                        initdir, datei.strftime('%Y%m%d'),
                        'savestate_%s.nc' % datei.strftime('%Y%m%d'))):
                datemin = datei
                found_datemin = True

            datei = date + dt.timedelta(days=d)
            if not found_datemax and os.path.exists(
                    os.path.join(
                        initdir, datei.strftime('%Y%m%d'),
                        'savestate_%s.nc' % datei.strftime('%Y%m%d'))):
                datemax = datei
            ...
            if found_datemin and found_datemax:
                print('Found datemin = %s and datemax = %s' %
                      (datemin.strftime('%Y%m%d'), datemax.strftime('%Y%m%d')))
                break
        del d
        logging.debug('Ensemble for %s will be interpolated from %s and %s' %
                      (date.strftime('%Y-%m-%d'), datemin.strftime('%Y-%m-%d'),
                       datemax.strftime('%Y-%m-%d')))

        # Read ensemble from both files
        filename1 = os.path.join(
            initdir, datemin.strftime('%Y%m%d'),
            'savestate_%s.nc' % datemin.strftime('%Y%m%d'))
        f = io.ct_read(filename1, 'read')
        meanstate1 = f.get_variable('statevectormean_' +
                                    qual)  # [nlag x nparameters]
        if readensemble:
            ensmembers1 = f.get_variable(
                'statevectorensemble_' +
                qual)  # [nlag x nmembers x nparameters]
        f.close()

        filename2 = os.path.join(
            initdir, datemax.strftime('%Y%m%d'),
            'savestate_%s.nc' % datemax.strftime('%Y%m%d'))
        f = io.ct_read(filename2, 'read')
        meanstate2 = f.get_variable('statevectormean_' +
                                    qual)  # [nlag x nparameters]
        if readensemble:
            ensmembers2 = f.get_variable(
                'statevectorensemble_' +
                qual)  # [nlag x nmembers x nparameters]
        f.close()

        # interpolate mean and ensemble between datemin and datemax
        meanstate = ((datemax - date).days / ddays) * meanstate1 + (
            (date - datemin).days / ddays) * meanstate2
        if readensemble:
            ensmembers = ((datemax - date).days / ddays) * ensmembers1 + (
                (date - datemin).days / ddays) * ensmembers2
            return meanstate, ensmembers

        else:
            return meanstate

    def read_mean_from_file(self,
                            filename,
                            lag,
                            date=None,
                            initdir=None,
        ...
        if date is None:
            ...
            meanstate = f.get_variable('statevectormean_' +
                                       qual)  # [nlag x nparameters]
            f.close
        else:
            meanstate = self.interpolate_mean_ensemble(initdir,
                                                       date,
                                                       qual,
                                                       readensemble=False)

        logging.info(
            'Successfully read the mean state vector from file (%s) ' %
            filename)

        return meanstate[lag, :]

    def read_ensemble_member_from_file(self,
                                       filename,
                                       lag,
                                       date=None,
                                       initdir=None,
        ...
        if date is None:
            ...
            meanstate = f.get_variable('statevectormean_' +
                                       qual)  # [nlag x nparameters]
            ensmembers = f.get_variable(
                'statevectorensemble_' +
                qual)  # [nlag x nmembers x nparameters]
            f.close()

        else:
            meanstate, ensmembers = self.interpolate_mean_ensemble(
                initdir, date, qual, readensemble=True)
        ...
        for m in range(self.nmembers):
            ...
            newmember.param_values = ensmembers[read_lag, m, :].flatten(
            ) + meanstate[
                read_lag, :]  # add the mean to the deviations to hold the full parameter values
            self.ensemble_members[lag].append(newmember)

        logging.info(
            'Successfully read the State Vector for lag %s from file (%s) ' %
            (lag, filename))
    ...
    def read_from_file(self, filename, qual='opt'):
        ...
        for n in range(self.nlag):
            ...
            for m in range(self.nmembers):
                newmember = EnsembleMember(m)
                newmember.param_values = ensmembers[n, m, :].flatten(
                ) + meanstate[
                    n]  # add the mean to the deviations to hold the full parameter values
                self.ensemble_members[n].append(newmember)

        logging.info('Successfully read the State Vector from file (%s) ' %
                     filename)
    ...
    def write_members_to_file(self,
        ...
        for mem in members:
            filename = os.path.join(
                outdir, 'parameters.%03d%s' % (mem.membernumber, endswith))
            ...
            savedict[
                'long_name'] = "parameter_values_for_member_%d" % mem.membernumber
            savedict['units'] = "unitless"
            savedict['dims'] = dimparams
            savedict['values'] = data
            savedict[
                'comment'] = 'These are parameter values to use for member %d' % mem.membernumber
            ...
            savedict[
                'long_name'] = "parametermap_for_member_%d" % mem.membernumber
            savedict['units'] = "unitless"
            savedict['dims'] = dimgrid
            savedict['values'] = griddata.tolist()
            savedict[
                'comment'] = 'These are gridded parameter values to use for member %d' % mem.membernumber
            ncf.add_data(savedict)
