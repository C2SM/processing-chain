...
#            covariancematrix = np.identity((self.nparams))
            ...
            for ix, x in enumerate(distances):
                for ic, c in enumerate(Corr):
                    for ik, k in enumerate(c):
                        if ic == 1 or ik == 1:
                            covariancematrix[ix*categories + ic,ik:-n_bg_params][::categories] = 0.5*np.exp(-x/specific_length_anth)*k
                        else:
                            covariancematrix[ix*categories + ic,ik:-n_bg_params][::categories] = 1*np.exp(-x/specific_length_bio)*k
            ...
            if n_bg_params>0:
                for iii in np.arange(n_bg_params):
                    ...
                    covariancematrix[-n_bg_params+np.mod(iii+1,n_bg_params),-n_bg_params+iii] = 0.015*0.015*0.25
                    covariancematrix[-n_bg_params+np.mod(iii-1,n_bg_params),-n_bg_params+iii] = 0.015*0.015*0.25
