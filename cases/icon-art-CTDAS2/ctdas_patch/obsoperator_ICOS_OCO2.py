...
class ObservationOperator(object):
    ...
    def __init__(self, rc_filename, dacycle=None): # David: addition arg "rc_filename" added.
    ...
    def prepare_run(self,samples):
        ...
        for i in range(len(samples)):
               ...
               logging.info("Simulated flask file added: %s"%self.simulated_file[i])
        del i
    ...
    def make_lambdas(self,statevector,lag):
        ...
        if statevector.isOptimized:
            self.lambda_file = os.path.join(self.simulationdir,'input','oae','lambda_%s_opt.nc' %self.dacycle['time.sample.stamp'][0:10])
            self.bg_lambda_file = os.path.join(self.simulationdir,'input','oae','bg_lambda_%s_opt.nc' %self.dacycle['time.sample.stamp'][0:10])
        else:
            if lag==0:
                self.lambda_file = os.path.join(self.simulationdir,'input','oae','lambda_%s_priorcycle1.nc' %self.dacycle['time.sample.stamp'][0:10])
                self.bg_lambda_file = os.path.join(self.simulationdir,'input','oae','bg_lambda_%s_priorcycle1.nc' %self.dacycle['time.sample.stamp'][0:10])
            else:
                self.lambda_file = os.path.join(self.simulationdir,'input','oae','lambda_%s_prior.nc' %self.dacycle['time.sample.stamp'][0:10])
                self.bg_lambda_file = os.path.join(self.simulationdir,'input','oae','bg_lambda_%s_prior.nc' %self.dacycle['time.sample.stamp'][0:10])
        ...
        odata = ofile.createVariable('lambda', np.float32, ('ens','reg','cat','tracer'),fill_value=-999.99)
        ...
        for m in range(0,self.forecast_nmembers):
            ...
            for ireg in range(0,nr_reg):
                for icat in range(0,nr_cat):
                    if statevector.isOptimized:
                        lambdas[m,ireg,icat,0] = members[0].param_values[param_count]
                    else:
                        lambdas[m,ireg,icat,0] = members[m].param_values[param_count]
                    param_count+=1
        odata[:] = lambdas
        ofile.close()
        logging.info('lambdas for ICON simulation written to the file: %s' % self.lambda_file)
        ...
        # otracer = ofile.createDimension('tracer', nr_tracer)
        odata = ofile.createVariable('lambda', np.float32, ('ens','reg'),fill_value=-999.99) #,'tracer'
        lambdas = np.empty(shape=(nr_ens,nr_dir)) #,nr_tracer
        for m in range(0,self.forecast_nmembers):
            for idir in range(0,nr_dir):
                if statevector.isOptimized:
                    lambdas[m,idir] = members[0].param_values[-self.n_bg_params + idir]
                else:
                    lambdas[m,idir] = members[m].param_values[-self.n_bg_params + idir]
        odata[:] = lambdas
        ofile.close()
        logging.info('bg_lambdas for ICON simulation written to the file: %s' % self.bg_lambda_file)
    ...
    def run(self,samples,statevector,lag):
        ...
        if statevector.isOptimized:
            ...
            runscript = os.path.join(self.simulationdir,'run','runscript_%sopt'%(self.dacycle['time.sample.stamp'][0:10]))
            #runscript_boundaries = os.path.join(self.simulationdir,'run','runscript_%sopt'%(self.dacycle['time.sample.stamp'][0:10]))
            extraction_script = os.path.join(self.simulationdir,'run','extract_%sopt'%(self.dacycle['time.sample.stamp'][0:10]))
            ...
            extracted_file = os.path.join(self.simulationdir,'extracted','output_%s_opt'%(self.dacycle['time.sample.stamp'][0:10]))
        else:
            if lag==0:
                runscript = os.path.join(self.simulationdir,'run','runscript_%spriorcycle1'%(self.dacycle['time.sample.stamp'][0:10]))
                extraction_script = os.path.join(self.simulationdir,'run','extract_%spriorcycle1'%(self.dacycle['time.sample.stamp'][0:10]))
                extracted_file = os.path.join(self.simulationdir,'extracted','output_%s_priorcycle1'%(self.dacycle['time.sample.stamp'][0:10]))
            else:
                runscript = os.path.join(self.simulationdir,'run','runscript_%sprior'%(self.dacycle['time.sample.stamp'][0:10]))
                extraction_script = os.path.join(self.simulationdir,'run','extract_%sprior'%(self.dacycle['time.sample.stamp'][0:10]))
            ...
            #icon_path = os.path.join(self.simulationdir,'output_%s_prior'%(self.dacycle['time.sample.stamp'][0:10]))
                extracted_file = os.path.join(self.simulationdir,'extracted','output_%s_prior'%(self.dacycle['time.sample.stamp'][0:10]))
        runscript_boundaries = os.path.join(self.simulationdir,'run_bg','runscript_boundaries%spriorcycle1'%(self.dacycle['time.sample.stamp'][0:10]))
        extraction_script_boundaries = os.path.join(self.simulationdir,'run_bg','extract_boundaries%spriorcycle1'%(self.dacycle['time.sample.stamp'][0:10]))
        extracted_boundaries_ens_file = os.path.join(self.simulationdir,'extracted','output_bg_%s_priorcycle1'%(self.dacycle['time.sample.stamp'][0:10]))
        logging.info('extraction_script: %s'%(extraction_script))

        template = os.path.join(self.simulationdir,'run','templates','sbatch_extract_template')
        sbatch_script = os.path.join(self.simulationdir,'run','sbatch_script')
        sbatch_script_bg = os.path.join(self.simulationdir,'run_bg','sbatch_script')
        # Write sbatch file
...
###########################################################
        ...
        molar_mass = {{'ch4' : 16.04e-3,
                    'co2' : 44.01e-3,          
                    'da'  :  28.97e-3           
                                    }}
        units_factor = {{'ch4' : 1.e9, #ppb for ch4
        ...
        mountain_stations = ['Jungfraujoch_5',
                            'Monte Cimone_8',
                            'Puy de Dome_10', 
                            'Pic du Midi_28',
                            'Zugspitze_3',
                            'Hohenpeissenberg_50',
                            'Hohenpeissenberg_93',
                            'Hohenpeissenberg_131',
                            'Schauinsland_12',
                            'Plateau Rosa_10'] 
        skip_stations = ['Malin Head_47', 
        ...
        TR_A_ENS = (molar_mass['da']/molar_mass[self.tracer])*units_factor[self.tracer]*np.array(f1.get_variable('TR'+self.tracer.upper()+'_A_ENS') + f1.get_variable('biosource_all_chemtr') - f1.get_variable('biosink_chemtr')) #float CH4_A_ENS(ens, sites, time) 1 --> ppb
        ...
        for iobs in np.arange(len(obs)):
            station_name = fromfile[iobs][fromfile[iobs]!=b''].tostring().decode('utf-8')
            if station_name in skip_stations: continue # Skip stations outside of the domain!
...
#                    print('DEBUG checking otime: ',otime,flush=True)
                ...
                if station_name in mountain_stations:
                    print('DEBUG station',station_name, 'is a mountain site',flush=True)
                    delta_index = obs_date.hour
                    print('DEBUG delta_index: ',delta_index,flush=True)
                else:
                    print('DEBUG station',station_name, 'is NOT a mountain site',flush=True)
...
#                        print('DEBUG looking through sampled stations. Checking site_name: ',site_name,flush=True)
                    if (site_name==station_name):
                        print('DEBUG looking through sampled stations. Found site_name: ',site_name,flush=True)
                        for iens in np.arange(self.forecast_nmembers):
                            if station_name in mountain_stations:
                                simulated_values[iobs,iens] = np.nanmean(TR_A_ENS[iens,isite,itime-delta_index:itime-delta_index+7])
                            else:
                                simulated_values[iobs,iens] = np.nanmean(TR_A_ENS[iens,isite,itime-delta_index:itime-delta_index+5])
                            if iens==50:
                                print('Added model value for member 0 of %.2f for iobs %i at %s at %s with a delta idx of %i'%(simulated_values[iobs,0],iobs,site_name,obs_date,delta_index))
                                print('Added model value for member 50 of %.2f for iobs %i at %s at %s with a delta idx of %i'%(simulated_values[iobs,50],iobs,site_name,obs_date,delta_index))
                        break
                    else:
                        continue
                break
###########################################################
    ...
    def _launch_icon_column_sampling(self, j, sample):
        ...
        run_dir = os.path.join(self.simulationdir,os.path.basename(self.extracted_file))
        ...
        for nproc in range(nprocs):
            cmd = " ".join([
                command_,
                "python ./da/tools/icon/icon_sampler.py",
                "--nproc %d"                 % nproc,
                "--nprocs %d"                % nprocs,
