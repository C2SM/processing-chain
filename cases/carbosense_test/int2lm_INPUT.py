...
!   Settings for GLOBE orography data
!   lfilter_oro = .FALSE,   ! if extpar orography is already filtered, set to .FALSE.
...
!   Settings for ASTER orography data   
    lfilter_oro = .FALSE,   ! if extpar orography is already filtered, set to .FALSE.
    ilow_pass_oro = 1,      ! type of low-pass filter for orography
    numfilt_oro = 2,        ! number of sequential applications of filter
    ilow_pass_xso = 5,      ! type of low-pass filter for extra smoothing steep oro.
    lxso_first = .FALSE.,   ! if .TRUE., do extra smoothing of orography first
    numfilt_xso = 1,        ! number of sequential applications of xso-filter
    rxso_mask = 750.0,      ! mask for extra smoothing of steep oro.: dh > rxso_mask
    eps_filter = 1.7,       ! parameter for orography filtering
    rfill_valley = 0.0,     ! mask for valley filling: dh > rfill_valley
    ifill_valley= 7,        !
    norder_filter = 5,      ! order of the orography filtering
...
! COSMO-1 Carbosense domain
  ...
  jelm_tot     = 600,
  kelm_tot     = 60, 
  ivctype = 4,                ! used by MeteoSwiss COSMO-7 -> 1 processing
